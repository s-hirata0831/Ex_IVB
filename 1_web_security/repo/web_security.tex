...
\begin{tabular}{|l|r|r|} \hline
  項目  & クッキー & セッション \\ \hline
  データ格納箇所 & クライアント(ブラウザ) & サーバ \\ \hline
  保存できるデータ量 & 少量 & 大量(サーバのリソースに依存) \\ \hline
...
実験で実装したエスケープ処理により，HTMLの特殊文字（\texttt{<}，\texttt{>}，\texttt{\&}など）をエンティティに変換することで，スクリプトがコードとして実行されることを防ぐことができた．
これは最も基本的で効果的なXSS対策の一つであると考える．
